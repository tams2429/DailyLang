...
function App() {
  const { phrases, currentIndex, status, error, loadPhrases, nextPhrase, previousPhrase } =
    usePhraseStore();
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 px-4 py-10">
      <div className="mx-auto flex max-w-2xl flex-col items-center gap-6">
        <header className="text-center">
          <h1 className="text-3xl font-bold text-slate-900">DailyLang</h1>
          <p className="mt-1 text-slate-500">Listen and practice a Japanese phrase every day</p>
        </header>

        {status === "loading" && <p className="text-slate-500">Loading phrases...</p>}
        {status === "error" && (
          <p className="text-rose-600">Failed to load phrases: {error}</p>
        )}
